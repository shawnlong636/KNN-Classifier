...
class SelectionAlgorithm(ABC):
    ...
    def possible_choices(self, features: list, max_features: int):
        pass
...
class ForwardSelection(SelectionAlgorithm):
    ...
    def search(self, validator: validator.Validator, num_features: int, display_text = True):
        ...
        try:
            ...
            if display_text:
                print(f"Features: {[]} -> Accuracy: {round(100.0 * best_accuracy) / 100.0 }%")
        ...
        while not all_children_worse:
            ...
            for choice in choices:
                ...
                try:
                    ...
                    if display_text:
                        print(f"Features: {best_feature_set + [choice]} -> Accuracy: {round(100.0 * child_accuracy) / 100.0 }%")
                except Exception as error:
                    if display_text:
                        print(f"Unable to complete feature search: {error}")
                    return
            ...
            if all_children_worse and display_text:
...
class BackwardElimination(SelectionAlgorithm):
    def search(self, validator: validator.Validator, num_features: int, display_text = True):
        ...
        try:
            best_accuracy = validator.evaluate(best_feature_set)
            if display_text:
                print(f"Features: {best_feature_set} -> Accuracy: {round(100.0 * best_accuracy) / 100.0 }%")
        except Exception as error:
            if display_text:
                print(f"Unable to complete feature search: {error}")
        all_children_worse = False
        ...
        while not all_children_worse:
            ...
            for feature in best_feature_set:
                ...
                try:
                    ...
                    if display_text:
                        print(f"Features: {child_features} -> Accuracy: {round(100.0 * child_accuracy) / 100.0 }%")
